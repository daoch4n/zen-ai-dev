...
export async function getEffectiveModel(
  ...
  const baseUrl = process.env.GEMINI_API_ENDPOINT || process.env.GOOGLE_GENAI_ENDPOINT || 'https://generativelanguage.googleapis.com';
  const endpoint = `${baseUrl}/v1beta/models/${modelToTest}:generateContent?key=${apiKey}`;
